...
def process_result(uploaded_file, subject_inputs=None, teacher_inputs=None):
    ...
    # Apply dynamic mappings from web
    if subject_inputs:
    ...
    while i < len(lines):
        ...
        if re.match(r"^\d{8}", line):
            ...
            mapped_subjects = subject_names[:len(marks_list)]
            marks = dict(zip(mapped_subjects, marks_list))
            grades = dict(zip(mapped_subjects, grades_list))
    ...
    header = ["Roll No", "Name"] + all_subjects + [
        "Main5 Total", "Main5 Percentage",
        "Best5 Total", "Best5 Percentage"
    ]
    sheet.append(header)
    ...
    for subject in all_subjects:
        ...
        grade_counts = {"A1":0,"A2":0,"B1":0,"B2":0,"C1":0,"C2":0,"D1":0,"D2":0,"E":0}
        total_students_subject = 0

        for roll, info in students.items():
            if subject in info["Grades"]:
                g = info["Grades"][subject]
        ...
        grade_percent = {
            g: round((grade_counts[g] / total_students_subject) * 100, 2)
        ...
        teacher_score = round(
            (
                ...
                2*grade_counts["D2"] +
                1*grade_counts["E"]
            ) / (9 * total_students_subject) * 100, 2
        )

        teacher_sheet.append([
            sno,
            subject,
            teacher_map.get(subject, ""),
            grade_counts["A1"], grade_percent["A1"],
            grade_counts["A2"], grade_percent["A2"],
            grade_counts["B1"], grade_percent["B1"],
            grade_counts["B2"], grade_percent["B2"],
            grade_counts["C1"], grade_percent["C1"],
            grade_counts["C2"], grade_percent["C2"],
            grade_counts["D1"], grade_percent["D1"],
            grade_counts["D2"], grade_percent["D2"],
            grade_counts["E"], grade_percent["E"],
            teacher_score
        ])
    ...
    doc.add_paragraph("(Session 2024-25)")
    doc.add_paragraph("")
    ...
    for roll, info in students.items():
        ...
        if is_absent:
            continue   # do NOT include absent students
    ...
    doc.add_page_break()
    # ===== FORM D : TOP 25 STUDENTS =====
    doc.add_page_break()
    doc.add_heading("Form D", level=1).runs[0].font.size = Pt(22)
    doc.add_paragraph("Name of the School : SPS TDSC")
    doc.add_paragraph("Details about TOP 25 students/rankers")
    doc.add_paragraph("AGGREGATE MARKS OUT OF 500 (Main 5 Subjects)")
    doc.add_paragraph("")
    ...
    for roll, info in students.items():
        ...
        # count failed subjects in main 5
        main5_subjects = list(marks.keys())[:5]
        main5_marks = [marks[s] for s in main5_subjects]
    ...
    hdr[2].text = "Aggregate Marks (out of 500)"
    hdr[3].text = "%"
    ...
    doc.add_paragraph("Signature of School Principal".ljust(60) + "Signature of Exam I/C")
    doc.add_page_break()
    # ===== FORMAT A : CONSOLIDATED ANALYSIS =====
    doc.add_page_break()
    ...
    doc.add_paragraph("CBSE RESULT - Session 2024-25")
    doc.add_paragraph("")
    ...
    for roll, info in students.items():
        ...
        marks = info["Marks"]
        grades = list(info["Grades"].values())[:5]
        ...
        # ---- Grade counting (Main 5 only) ----
        for g in grades:
            if g in grade_count:
                grade_count[g] += 1

        if grades.count("A1") == 5:
            A1_inall_sub += 1

        # ---- Main5 Percentage tracking ----
        ...
        # ---- PASS / COMPARTMENT / FAIL ----
        main5_subjects = list(marks.keys())[:5]
        main5_marks = [marks[s] for s in main5_subjects]
        highest_marks = max([info["Main5_Total"] for roll, info in eligible_students], default=0)
    ...
    rows_data = [
        ...
        ("3", "Passed", present_students - failed_students_count - compartment_students, round(((present_students - failed_students_count - compartment_students)/present_students)*100,2) if present_students>0 else 0),
        ("4", "Compartment", compartment_students, round(compartment_students/present_students*100,2) if present_students>0 else 0),
        ("5", "Failed", failed_students_count, round(failed_students_count/present_students*100,2) if present_students>0 else 0),
        ...
        ("15", "Highest % (Main 5)", highest_marks, highest_percent),
        ("16", "A1 in all subjects", A1_inall_sub, ""),
